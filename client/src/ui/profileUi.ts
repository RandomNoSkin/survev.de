...
export class ProfileUi {
    ...
    private showNextSale() {
        ...
        $("#modal-sale-notify-detail").html(
            `Sold for <b>${sale.price}</b> <span class="sale-fries-icon"></span> to ` +
                helpers.formatUsername(sale.buyerName, sale.buyerPremium),
        );
    ...
    private showNextGift() {
        ...
        $("#modal-gift-notify-detail").html(
            `from <b>${helpers.formatUsername(gift.fromName, gift.fromPremium)}</b>`,
        );
        // Re-open after a tick so the fadeOut from the previous card can settle.
    ...
    private showNextAuction() {
        ...
        if (a.kind === "won") {
            ...
            detail = `Won for <b>${a.amount}</b> 🍟 from ${helpers.formatUsername(a.otherName, a.otherPremium)}`;
        } else if (a.kind === "sold") {
            title = "Your auction sold!";
            detail = `Sold for <b>${a.amount}</b> 🍟 to ${helpers.formatUsername(a.otherName, a.otherPremium)}`;
        } else {
            title = "Auction ended";
