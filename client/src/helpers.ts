    ...
    getSvgFromGameType: function(gameType: string) {
        ...
        switch (def.type) {
            ...
            case "outfit": {
                ...
                const iconUrl = getOutfitIconUrl(gameType);
                if (iconUrl) {
                    return iconUrl;
                }

                // Atlas not ready yet (or a costume/ghillie skin): fall back to a shirt
                // tinted with the skin's colour. A white tint means "untinted", so use
                // the sprite's own art (e.g. the default tan Basic Outfit shirt).
                if (lootImg.tint === 0xffffff) {
                    return `img/loot/${lootImg.sprite.slice(0, -4)}.svg`;
                }

                // tint outfits using the generic shirt shape
